...
def propertyType(data_str):
    data_str = str(data_str).lower().replace(",_", "_").replace(
...
df["hf_parking"] = df["hf_parking"].apply(
    lambda x: str(x).lower().replace("one", "1"))
df["hf_parking"] = df["hf_parking"].apply(
    lambda x: str(x).lower().replace("two", "2"))
df["hf_parking"] = df["hf_parking"].apply(
    lambda x: str(x).lower().replace("parking", "1"))
df["hf_parking"] = df["hf_parking"].apply(
    lambda x: str(x).lower().replace("garage", "1"))
...
df["private pool"] = df["private pool"].apply(
    lambda x: True if x == 'yes' else False)
df["PrivatePool"] = df["PrivatePool"].apply(lambda x: str(x).lower())
df["PrivatePool"] = df["PrivatePool"].apply(
    lambda x: True if x == 'yes' else False)
df["priv_pool"] = df["private pool"] + df["PrivatePool"]

# @title Droping redundant columns

df = df.drop(['propertyType', "MlsId", "mls-id", "street",
             "private pool", "PrivatePool"], axis=1)
...
def schools_data(data_str):
    ...
    try:
        ...
        school_grades = [str(each).lower().replace("preschool", "p").replace(
            " to ", "-").replace('none', '').replace("n/a", '') for each in school_grades]
        if len(school_grades) == 0:
            return pd.NA
        school_ratings = list_of_dicts[0]["rating"]
        school_ratings = [str(each).lower().replace('none', '').replace(
            "nr", '').replace('na', '') for each in school_ratings]
        if "/" in "x".join(school_ratings):
            for idx, each_rate in enumerate(school_ratings):
                ...
                try:
                    school_ratings[idx] = int(
                        tmp_nums[0]) / int(tmp_nums[1]) * 10
                except Exception as e:
                    school_ratings[idx] = ''
        if len([s for s in school_ratings if s]) == 0:
            return pd.NA
        school_distances = list_of_dicts[0]["data"]["Distance"]
        school_distances = [str(each).lower().replace(
            'mi', '').replace(" ", "") for each in school_distances]
...
print(
    f'Кол-во строк с неконсистентными данными о школах: {round(df["schools_data"].isna().sum()/len(df) * 100 , 2 )}%')
...
def rollout_school_data(data_tuple):
    ...
    rating_mean = sum([np.float16(item) for item in rating_list if item != ""]
                      )/len([item for item in rating_list if item != ""])

    grades_ranges = data_tuple[0]

    distnce_list = data_tuple[1]
    distnce_mean = sum([np.float16(item) for item in distnce_list if item != ""]
                       )/len([item for item in distnce_list if item != ""])
...
for each in grades_list:
    list_of_school_data_feilds = list_of_school_data_feilds + \
        [f"gr_{each}_top_rate_dist", f"gr_{each}_top_rate", f"gr_{each}_min_dist"]

list_of_school_data_feilds = list(
    map(lambda x: f"schl_{x}", list_of_school_data_feilds))

df[list_of_school_data_feilds] = df["schools_data"].apply(rollout_school_data)

# @title Dropping useless and redundant columns

df = df.drop(['zipcode', 'schools', 'hf_price_sqft',
             'hf_price_sqft', 'schools_data'], axis=1)

# @title Converting Year columns to age data

current_year = datetime.now().year
df["hf_year_built"] = df["hf_year_built"].apply(
    lambda x: 0 if x == 'no data' else x)
df["hf_year_built"] = current_year - df["hf_year_built"].fillna(0).astype(int)
df["hf_year_built"] = df["hf_year_built"].apply(
    lambda x: pd.NA if x == current_year else x)

df["hf_remodeled_year"] = df["hf_remodeled_year"].apply(
    lambda x: 0 if x == 'no data' else x)
df["hf_remodeled_year"] = current_year - \
    df["hf_remodeled_year"].fillna(0).astype(int)
df["hf_remodeled_year"] = df["hf_remodeled_year"].apply(
    lambda x: pd.NA if x == current_year else x)
...
df["fireplace"] = df["fireplace"].apply(
    lambda x: 1 if str(x).lower() == 'yes' else x)
df["fireplace"] = df["fireplace"].apply(
    lambda x: 1 if 'fireplace' in str(x).lower() else x)
df["fireplace"] = df["fireplace"].apply(
    lambda x: 1 if 'room' in str(x).lower() else x)
df["fireplace"] = df["fireplace"].apply(
    lambda x: 1 if 'wood' in str(x).lower() else x)
df["fireplace"] = df["fireplace"].apply(
    lambda x: 1 if 'one' in str(x).lower() else x)
df["fireplace"] = df["fireplace"].apply(
    lambda x: 1 if 'logs' in str(x).lower() else x)
df["fireplace"] = df["fireplace"].apply(
    lambda x: 0 if str(x).lower() == 'no' else x)
df["fireplace"] = df["fireplace"].apply(
    lambda x: 0 if "not applicable" in str(x).lower() else x)
...
df["status"] = df["status"].apply(
    lambda x: "coming_soon" if "coming_soon" in str(x) else x)
df["status"] = df["status"].apply(
    lambda x: "pre_foreclosure" if "pre-foreclosure" in str(x) else x)
df["status"] = df["status"].apply(
    lambda x: "pre_foreclosure" if "auction" in str(x) else x)
df["status"] = df["status"].apply(
    lambda x: "pre_foreclosure" if str(x) == "p" else x)
df["status"] = df["status"].apply(
    lambda x: "under_contract" if "under_contract" in str(x) else x)
...
df["hf_heating"] = df["hf_heating"].apply(
    lambda x: str(x).lower().replace(" ", "_"))
df["hf_heating"] = df["hf_heating"].apply(
    lambda x: "gas" if "gas" in str(x) else x)
df["hf_heating"] = df["hf_heating"].apply(
    lambda x: "heat_pump " if "pump" in str(x) else x)
df["hf_heating"] = df["hf_heating"].apply(
    lambda x: "wood" if "wood" in str(x) else x)
df["hf_heating"] = df["hf_heating"].apply(
    lambda x: "electric" if "electric" in str(x) else x)
df["hf_heating"] = df["hf_heating"].apply(
    lambda x: "central" if "central" in str(x) else x)
df["hf_heating"] = df["hf_heating"].apply(
    lambda x: "air" if "air" in str(x) else x)

heating = df['hf_heating'].value_counts().head(10)
allowed_heating = heating.keys()
df['hf_heating'] = df['hf_heating'].where(
    df['hf_heating'].isin(allowed_heating), pd.NA)

# @title Normolazing hf_cooling column

df["hf_cooling"] = df["hf_cooling"].apply(
    lambda x: str(x).lower().replace(" ", "_"))
df["hf_cooling"] = df["hf_cooling"].apply(
    lambda x: "gas" if "gas" in str(x) else x)
df["hf_cooling"] = df["hf_cooling"].apply(
...
df["hf_cooling"] = df["hf_cooling"].apply(
    lambda x: "central" if "central" in str(x) else x)
df["hf_cooling"] = df["hf_cooling"].apply(
    lambda x: pd.NA if str(x) == '""' else x)

cooling = df['hf_cooling'].value_counts().head(10)
allowed_cooling = cooling.keys()
df['hf_cooling'] = df['hf_cooling'].where(
    df['hf_cooling'].isin(allowed_cooling), pd.NA)
...
def convert_cat_col(df, tgt_col):
    ...
    if len(cat_stat.keys()) > 40:
        print(
            f"Column [{tgt_col}] has {len(cat_stat.keys())} categories, which is more than 20. This may lead to overfitting.")
...
high_threshold_value = np.percentile(
    df[target_column], high_percentile_threshold)
...
# Calculate the threshold value based on the specified percentile
low_threshold_value = np.percentile(
    df[target_column], low_percentile_threshold)
...
X_train, X_val, y_train, y_val = train_test_split(
    X_rest, y_rest, test_size=0.1, random_state=42)
...
catboost_model.fit(X_train, y_train, eval_set=eval_set,
                   early_stopping_rounds=50, verbose=25)
